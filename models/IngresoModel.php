...
class IngresoModel extends Model {
    ...
    public function crearIngreso($usuario_id, $monto, $categoria, $descripcion, $fecha, $cuenta_id = null, $es_transferencia = 0, $transferencia_id = null) {
        if (!$this->conn) return false;
        if (empty($usuario_id) || empty($monto) || empty($categoria) || empty($fecha)) {
            return false;
        }
        try {
            $sql = "INSERT INTO ingresos (usuario_id, cuenta_id, es_transferencia, transferencia_id, monto, categoria, descripcion, fecha) VALUES (:usuario_id, :cuenta_id, :es_transferencia, :transferencia_id, :monto, :categoria, :descripcion, :fecha)";
