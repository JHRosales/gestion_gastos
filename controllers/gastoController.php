...
class GastoController extends Controller {
    ...
    public function editar($id = null) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $ok = $this->_modelo->actualizarGasto($id, $usuario_id, $monto, $categoria, $descripcion, $fecha);
            if ($ok) {
    ...
    public function registrar() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $ok = $this->_modelo->crearGasto($usuario_id, $monto, $categoria, $descripcion, $fecha);
            if ($ok) {
                $_SESSION['success_gasto'] = 'Gasto registrado correctamente.';
            } else {
                $_SESSION['error_gasto'] = 'Error al registrar el gasto.';
                $log = date('Y-m-d H:i:s') . " | Error al registrar gasto | usuario_id: $usuario_id | monto: $monto | categoria: $categoria | descripcion: $descripcion | fecha: $fecha\n";
                $logDir = __DIR__ . '/../logs';
                if (!is_dir($logDir)) {
